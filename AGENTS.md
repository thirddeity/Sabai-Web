...
* Keep each feature page as a thin shell that composes smaller local components.
* Split long or crowded JSX blocks into module-local components inside `components/` when the page starts to feel hard to scan.
* Prefer small, named subcomponents for repeated UI, distinct sections, or complex conditional blocks.
* Do not leave one page file responsible for all layout, state, logic, and markup if a clearer component split is available.
* Break long props and JSX lines across multiple lines so the file stays readable.
...
### UI Implementation Decision Order

Before writing frontend UI, choose styling in this order:

1. Use Ant Design components and props first, such as `Row`, `Col`, `Flex`, `Space`, `Card`, `Button`, and responsive props.
2. Use Tailwind only for small utilities such as `w-*`, `min-w-0`, `max-w-*`, `overflow-hidden`, or one-off spacing/detail tweaks.
3. Use feature CSS only for visual patterns or cases AntD/Tailwind do not handle cleanly, such as glass surfaces, gradients, pseudo-elements, animation, complex backgrounds, or repeated project-specific visual treatments.
4. Do not create CSS classes for grid/flex layout when AntD can express the layout directly.
...
```tsx
// Bad: CSS class duplicates an AntD responsive grid.
<div className="sabai-task-summary-grid" />

// Good: AntD owns the layout and breakpoints.
<Row gutter={[12, 12]}>
  <Col xs={24} sm={12} lg={6}>...</Col>
</Row>

// Good: CSS owns project visual treatment only.
...
### Thai Module Wording Rules

ทุก module ต้องใช้คำไทยที่สื่อความหมายกับสิ่งที่ผู้ใช้ทำจริง

* ชื่อหน้า ปุ่ม label empty state และคำอธิบายหลักต้องอ่านแล้วรู้ทันทีว่า module ใช้ทำอะไร
* หลีกเลี่ยงศัพท์เทคนิค คำกว้าง หรือคำอังกฤษใน UI หลัก ถ้ามีคำไทยที่เข้าใจง่ายกว่า
* ใช้คำที่ผูกกับงานของผู้ใช้ เช่น `งานที่ต้องทำ`, `เพิ่มงาน`, `ทำเสร็จแล้ว`, `ความสำคัญ`, `เอกสารสำคัญ`, `นัดหมาย`, `รายรับ`, `รายจ่าย`
* ถ้าชื่อ internal เป็นภาษาอังกฤษได้ แต่ visible UI ต้องใช้คำไทยที่เหมาะกับคนไทยหลายวัย
* ก่อนทำ module ใหม่ ต้องล็อกคำหลักของ module นั้นในเอกสารหรือ decision log ก่อน เพื่อให้หน้า route และปุ่มใช้คำตรงกัน
...
* รายการงานที่ต้องทำ
* เพิ่ม แก้ไข ลบ และทำเครื่องหมายว่าเสร็จแล้ว
* ความสำคัญที่ผู้ใช้เลือกเอง
* กรองงานค้าง งานสำคัญ งานที่เสร็จแล้ว และงานทั้งหมด
...
ถ้าเป็น frontend UI task ต้องสรุปเพิ่ม:

* layout ใช้ AntD component/props อย่างไร
* responsive ใช้ AntD responsive props หรือ `withBreakpoint` อย่างไร
* Tailwind ใช้เฉพาะ utility เล็ก ๆ ตรงไหน
* CSS class ใหม่จำเป็นเพราะอะไร และ AntD/Tailwind ไม่เหมาะตรงไหน
